...
import { getDecryptedVaultItems } from "@/extension/background.ts";
...
export async function handleSubmittedCredentials(
  rawCreds: unknown,
  tabId: number,
): Promise<void> {
...
export async function processPendingUnapprovedCredentials(): Promise<void> {
...
export async function handleCheckAutofillSuggestion(
  ...
  const domainStr = payload.domain;
  if (!domainStr) {
...
export async function handleCredentialsSubmitted(
  ...
  if (context.sender.tab && context.sender.tab.id !== undefined) {
    await handleSubmittedCredentials(
      payload.credentials,
      context.sender.tab.id,
    );
  }
