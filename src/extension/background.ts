...
import { uploadToGist, downloadFromGist, validateToken } from "@/domains/github/api.ts";
import { POPUP_WIDTH, FIDO2_PROMPT_HEIGHT } from "@/shared/constants.ts";
...
// Message listener
chrome.runtime.onMessage.addListener((message: ChromeMessage, sender: chrome.runtime.MessageSender, sendResponse: (response: unknown) => void) => {
  ...
  switch (message.type) {
    ...
    case "START_GITHUB_OAUTH": {
      ...
      const authUrl = `https://github.com/login/oauth/authorize?client_id=${clientId}&scope=gist,read:user&state=${encodeURIComponent(redirectUri)}`;
      ...
      chrome.identity.launchWebAuthFlow({
        ...
        if (chrome.runtime.lastError || !redirectUrl) {
          const err = chrome.runtime.lastError?.message || "OAuth flow cancelled or failed";
        ...
        try {
          ...
          if (token) {
            ...
            sendResponse({ success: false, error: "Token not found in redirect URL" });
          }
        } catch (e) {
          const errMsg = e instanceof Error ? e.message : String(e);
          sendResponse({ success: false, error: "Failed to parse redirect URL: " + errMsg });
        }
      });
      return true;
    }
    ...
    case "FIDO2_CREDENTIAL_GET_REQUEST": {
      const type = message.type === "FIDO2_CREDENTIAL_CREATION_REQUEST" ? "create" : "get";
    ...
    case "RESOLVE_FIDO2_REQUEST":
      if (pendingFido2Request) {
        pendingFido2Request.sendResponse({ success: true, result: message.result });
        pendingFido2Request = null;
        sendResponse({ success: true });
      } else {
        sendResponse({ success: false, error: "No pending request to resolve" });
      }
      return false;

    case "REJECT_FIDO2_REQUEST":
      if (pendingFido2Request) {
        pendingFido2Request.sendResponse({ success: false, error: message.error || "User cancelled" });
        pendingFido2Request = null;
        sendResponse({ success: true });
      } else {
        sendResponse({ success: false, error: "No pending request to reject" });
      }
      return false;
  ...
  try {
    ...
    try {
      ...
      if (url.protocol === "otpauth:" && url.searchParams.has("secret")) {
        ...
        return { success: false, error: "QR code is not a valid TOTP URL (missing secret)" };
      }
    } catch (_e) {
